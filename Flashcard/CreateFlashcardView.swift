...
struct CreateFlashcardView: View {
    ...
    var body: some View {
        VStack(spacing: 60) { // <-- Top level VStack, separates the button from the text fields
            ...
            // Adding my name and Z number
            Text("Aryan Lakhani")
                .font(.system(size: 24, weight: .bold, design: .rounded))
                .padding(.bottom, 10) // Adjust padding between the name and Z number

            Text("Z Number Z23724811")
                .font(.system(size: 24, weight: .bold, design: .rounded))
                .padding(.bottom, 80) 

            Spacer() // This spacer lifts the text up from the bottom further if needed
        }
        .padding()
    }
}
